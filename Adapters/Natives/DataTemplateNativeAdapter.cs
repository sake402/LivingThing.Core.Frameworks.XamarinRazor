...
namespace LivingThing.Core.Frameworks.XamarinRazor.Adapters.Natives
{
    ...
    internal class DataTemplateNativeAdapter : INativeAdapter<XRF.DataTemplate>
    {
        public bool Add(XRF.DataTemplate template, int index, object child, IComponentAdapterController rootAdapter, bool @throw = true)
        {
            ...
            //if (child is XF.Element element)
            {
                ...
                else if (template is DataTemplateTBase tbase && tbase.TemplateChildContent != null)
                {
                    newDataTemplate = new XF.DataTemplate(() =>
                    {
                ...
                else if (typeof(ComponentBase).IsAssignableFrom(template.Template))
                {
                    newDataTemplate = new XF.DataTemplate(() => Razor.Create<Element>(rootAdapter.ServiceProvider, template.Template).Result);
                }
                else
                {
                    newDataTemplate = new XF.DataTemplate(template.Template);
                }
            ...
            if (@throw)
                throw new InvalidOperationException($"{typeof(XF.DataTemplate)} doesn't supports only ChildContent of type {child.GetType()}");
            return false;
        }
